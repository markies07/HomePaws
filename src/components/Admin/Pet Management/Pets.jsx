...
function Pets({pets, selected, loading}) {
    ...
    return (
        <div className='px-3 py-6 inline-grid place-items-center gap-3 sm:gap-4 h-full w-full justify-center grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 md:px-5'>
            {pets.length > 0 ? (
                pets.map(pet => (
                    <div key={pet.petID} onClick={() => navigate(`/admin/pet-management/${pet.petID}`)} className='w-full mb-5 duration-150 cursor-pointer border-2 border-transparent hover:border-[#F75959] relative h-64 2xl:h-72 flex flex-col drop-shadow-md rounded-xl overflow-hidden'>
