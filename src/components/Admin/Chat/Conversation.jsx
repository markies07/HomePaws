        ...
        <div className='w-full h-full flex flex-col font-poppins'>
            ...
            <div className='bg-[#E9E9E9] w-full h-full flex flex-col flex-grow sm:rounded-lg'>
                <div className='flex justify-center items-center relative px-5 py-4 border-b-[1px] border-text'>
                    <img onClick={!isnewMessage ? () => navigate('/admin/chat') : handleCancelChat} className='absolute left-4 border-2 border-transparent hover:border-text duration-150 cursor-pointer p-1 w-10' src={back} alt="" />
                ...
                messages.map((message) => (
                    message.sender === user.uid ? (
                        ...
                        <div key={message.id} className='flex w-full justify-end'>
                            ...
                            {/* PROFILE */}
                            <img className='w-10 h-10 self-end object-cover shrink-0 rounded-full bg-text ml-3' src={userData.profilePictureURL} alt="" />
                        </div>
                    ) : (
                        // SENDER
                        <div key={message.id} className='flex w-full justify-start'>
                            ...
                            <div className='w-[60%]'>
                                {message.image ? ( // Check if there's an image
                                    <img src={message.image} onClick={() => showModal(message.image)} alt="received" className='max-w-full object-cover cursor-pointer rounded-lg mb-2' />
                                ) : (
                ...
                <form onSubmit={(e) => {e.preventDefault(); handleSendMessage();}} className='flex px-4 py-4 border-t-[1px] border-text'>
                    ...
                    <button type='submit' disabled={isSending} className='w-10 shrink-0 h-10 flex justify-center items-center overflow-visible bg-[#BCBCBC] hover:bg-[#adadad] cursor-pointer duration-150 rounded-full'>
