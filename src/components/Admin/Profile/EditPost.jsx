...
function EditPost({ postId, closeEdit }) {
    ...
    const handleImageUpload = async (e) => {
        ...
        if (files.length === 0) {
            notifyErrorOrange('Only 3 images can be uploaded.')
            return;
        };
        ...
        try {
            ...
            let newImages = [...images]; // Local variable to hold images
            ...
            for (const file of files) {
                ...
                newImages.push(downloadURL); // Add to local array
            }
    
            console.log("Updating state with:", newImages); // Log final array before state update
            setImages(newImages); // Only update state once
        } catch (error) {
            console.error("Error uploading images:", error);
            notifyErrorOrange('Error uploading images');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    // useEffect(() => {
    //     console.log('BAGONG IMAGES: ', images)
    // },[images])
