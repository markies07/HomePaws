...
function Contract({data}) {
    ...
    const adoptionContractTemplate = `
        ...
        Pet Information:
        - Name: ${data.petDetails.petName}
        - Type: ${data.petDetails.petType}
        - Age: ${data.petDetails.petAge}
        - Color: ${data.petDetails.color}
    ...
    const generateAdoptionContract = async () => {
      ...
      const generatePDF = async () => {
        ...
        doc.save(`${data.petDetails.petName}_AdoptionContract.pdf`);
    };
    ...
    return (
        <button onClick={() => generatePDF()} className='flex bg-primary cursor-pointer duration-150 hover:bg-primaryHover items-center text-xs md:text-base font-medium gap-2 text-white p-2 rounded-md'><img className='w-4 h-4' src={contract} alt="" />Adoption Contract</button>
    )
}
