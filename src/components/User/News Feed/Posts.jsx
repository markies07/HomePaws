                    ...
                    return (
                        <div key={post.id} className='bg-secondary relative w-full sm:rounded-lg shadow-custom py-5 px-5 md:px-7'>
                            ...
                            <div className='flex pb-1 w-full'>
                                ...
                                <div className='ml-2'>
                                    <p onClick={() => navigate(`/dashboard/profile/${post.userID}`)} className={`font-medium ${post.userID !== user.uid ? 'cursor-pointer' : 'pointer-events-none'}`}>{post.userName} <span className='text-xs sm:text-sm sm:px-3 font-normal ml-1 text-white rounded-full px-2' style={{backgroundColor: post.typeOfPost === 'story' ? '#A87CCD' : post.typeOfPost === 'missing' ? '#ED5050' : '#85B728'}}>{post.typeOfPost}</span></p>
